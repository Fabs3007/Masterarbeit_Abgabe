...
x_p2 = np.array([0.001,0.001,0.001])
x_k2 = np.array([0.001,0.001,0.001])
offset = 0.1516
X_P1 = 0
X_P2 = 0
plot_array = []
...
def ekf1(H,R1,P1):
    ...
    def ekf2(H, R1, P1):
        ...
        ax = acceleration2_x_np
        ay = acceleration2_y_np
        az = acceleration2_z_np
        x2_1 = gx + math.sin(x_k2[0]) * math.tan(x_k2[1]) * gy + math.cos(x_k2[0]) * math.tan(x_k2[1]) * gz
        x2_2 = math.cos(x_k2[0]) * gy - math.sin(x_k2[0]) * gz
        x2_3 = gy * (math.sin(x_k2[0]) / math.cos(x_k2[1])) + gz * (math.cos(x_k2[0]) / math.cos(x_k2[1]))

        x_p2[0] = x_k2[0] + deltat * x2_1
        x_p2[1] = x_k2[1] + deltat * x2_2
        x_p2[2] = x_k2[2] + deltat * x2_3

        a = 1 + deltat * (gy * math.cos(x_k2[0]) * math.tan(x_k2[1]) - gz * math.sin(x_k2[1]) * math.tan(x_k2[2]))
        b = deltat * (1 / (math.cos(x_k2[1])) ** 2) * math.cos(x_k2[0]) * gz + math.sin(x_k2[0]) * gy
        c = 0
        d = deltat * (-gy * math.sin(x_k2[0]) - gz * math.cos(x_k2[1]))
        e = deltat
        f = 0
        g = deltat * (gy * math.cos(x_k2[0]) / math.cos(x_k2[1]) - gz * math.sin(x_k2[0]) / math.cos(x_k2[1]))
        r = deltat * (gy * math.sin(x_k2[0]) * math.sin(x_k2[1]) / 2 * math.cos(x_k2[0]) ** 2 + gz * math.cos(
            x_k2[0]) * math.sin(x_k2[1]) / 2 * math.cos(x_k2[1]) ** 2)
        k = deltat
        F2 = np.array([[a, b, c], [d, e, f], [g, r, k]], dtype=float)
        P_k2 = np.dot(F, np.dot(P2, F2.T)) + Q2
        pitch = math.asin(ax / math.sqrt((ax ** 2) + (ay ** 2) + (az ** 2)))
        roll = math.asin(-ay / math.sqrt((ax ** 2) + (ay ** 2) + (az ** 2)) * math.cos(pitch))
        h = x_p2
        z2 = np.array([roll, pitch, 0])
        y2 = z2 - h
        s2 = np.dot(H2, np.dot(P_k2, H2.T)) + R2
        k2 = np.dot(P_k, np.dot(H2.T, np.linalg.inv(s2)))
        x_k2 = x_p2 + np.dot(k2, y2)
        P2 = np.dot(np.eye(3) - np.dot(k2, H2), P_k2)
...
Angular_FZ = THETA_PREDICT2 - THETA_PREDICT1
#
